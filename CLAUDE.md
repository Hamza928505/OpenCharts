...
- `board.html` — several saved charts on one page, and that page as one file.
  Deliberately not a dashboard; see "A board of several charts".
...
**And `dataio.js` reads `PALETTE` rather than keeping a copy.** It kept one, and
the copy drifted the moment the palette was corrected for colour-blind readers:
every series created by a paste or a new column was handed a colour from the
old colliding set, one at a time. Two lists that have to agree are one list.
...
swatch is offered — a swatch on the wrong thing is worse than none.

**Where they live is settled once; how many there are is not.** The grid asks
`modeFor` on every render, so adding a series brings its swatch with it rather
than waiting for the editor to be reopened, and a column with no colour yet
shows the one `applyData` will give it. Only colours somebody actually picked
are written back: a swatch showing a default has not been chosen by anyone, and
writing it would turn a default into a decision.

**`paletteOf` reports one colour as readily as eight.** It used to require two
for the series route, which is a colour-*vision* rule — a lone series has no
pair that can merge — and it meant every single-series chart looked to the
editor as though it had no colours at all, and got no Colour row. Callers that
need a pair check the length. The same
...
One chart at a time was the whole product, and the thing people ask for next is
a page of them. The obvious name for that is a dashboard and it is the wrong
one: a dashboard is a set of charts wired to each other, so clicking a region
filters every other chart, which needs one query engine, one data model and one
runtime holding all of them. This library's hard rule is that *a renderer reads
its data from the spec and nothing else*, and every exported chart would have
to carry that engine to reproduce numbers it could simply have been given.

**A board is the honest version.** N specs on one page, laid on a 1–4 column
grid, each rendered by the same `renderChart` the studio uses and each
exportable on its own. Nothing on a board can change anything else on it. That
is a smaller promise, and unlike the other one it survives being exported: a
board's standalone file is N independent charts in one document, not a page
that needs a server to mean anything. The README says so out loud, because
"dashboard" is what a reader will call it and the difference is the point.

`board.js` is the model and the exporter, with no page in it; `BoardApp.js` is
the page; `board.html` is the third page of the site. The charts come from the
shelf, and a board names entries **by their shelf id** rather than copying
their specs — so editing a chart in the studio and coming back shows the edit,
and removing it from My charts takes it off the board. That is the right way
round: a card whose chart no longer exists says so instead of drawing a stale
copy of it.

Three things borrowed rather than re-invented: the gallery's `_queue` / `_pump`
frame budget, for the gallery's reason — a board opens with every card in view,
and mounting twelve charts in one frame is twelve charts' worth of blocked main
thread; `captionHead` / `captionFoot`, so a board reads as a page rather than a
grid of unlabelled plates; and `encodeSpec`, since a board is much smaller JSON
than a single chart's spec. What the link does **not** carry is the charts —
it names entries, so opening it on another browser draws the cards it can and
says so for the rest.

#### Putting N exports in one document

Each chart's export owns its document. It says `id="chart"`, it styles `#chart`
and `.chart-wrap`, and its script calls `document.getElementById` — all correct
for one chart in one file and all colliding the moment there are three. Three
substitutions make them co-exist, and between them they leave every renderer's
own code **untouched**, which is the point:

- **Ids are prefixed per card** (`chart` → `c1-chart`), in the markup and in the
  `aria-describedby` that points at it, so the document stays valid. The ids are
  collected from the markup rather than listed, so a chart that starts emitting
  another one is carried without the exporter learning about it.
- **CSS is prefixed and then scoped** to the card's own box. `scopeCss` walks
  the stylesheet `buildCSS` wrote and puts `#c1-card` in front of every
  selector, recursing into `@media` and leaving `@keyframes` alone — because
  `#chart { min-height: 420px }` differs per chart and `def.css` is per chart,
  and either would otherwise land on all three. `html`, `body` and `:root`
  become the card itself: in a one-chart export they name the page, and on a
  board the page they mean is the card.
- **The script is handed a `document` that can only see its card.** A Proxy
  ...
  `querySelector` to it. That is what lets the generated JS be emitted
  *verbatim* — including the lookups it computes at run time (`'chart-' + i` on
  a faceted card) and the class selectors the annotation overlay and the facet
  grid use, neither of which a textual rewrite could have found.

The one place the proxy cannot reach is the custom engine, whose chart class
takes an id *string* and resolves it itself, inside the library. `BaseChart`
accepts an element too, so the constructor's argument becomes the element and
the lookup happens in the card's own `document` after all.

Library tags are deduped across cards — three Chart.js charts produce one
`<script>` — which is the whole reason the exporter assembles the parts rather
than concatenating whole pages.

The suite serves the exported board and requires it to run with no page errors,
draw every chart, and hold no duplicate id anywhere in the document. The PNG
takes the same route `StudioApp._exportGrid` does and for the same reason:
`querySelector('canvas')` would find card one and hand over a picture of a
third of the page with nothing saying so. `composite()` is separate from the
download so the suite can measure what was composited rather than a file it
cannot open.
...
| `board.js` | A board — what several saved charts on one page *is*, and the one HTML file it becomes |
| `BoardApp.js` | The board page — the picker, the cards, the reordering and the exports |
...
| Pattern | Meaning |
|---|---|
| `index.html` / `studio.html` / `board.html` | The three live pages |
